...
// Few notes about single producer single consumer queue. During desing phase there are few things
// to consider:
// 1. Can it have fixed size?
// - Yes, circular buffer can be used.
// - No, than process of reallocation has to be locked on both threads.
// For circular buffer power of two can be used to reduce number of operations. Since, mask can be
// applied to recuse about of bits for index calculation.

template <typename type_T, template <typename, typename> class impl_T = impl::queue_mutex_t,
...
template <typename type_T, template <typename, typename> class impl_T,
