...
TEST(NoVector, empty) {
    ...
    ASSERT_FLOAT_EQ(calculate_vwap_no_vector(ob), 0.0f);
}

TEST(NoVector, single_level) {
    const order_book_t ob{.m_prices = {100.0f}, .m_quantities = {10.0f}};
    ASSERT_FLOAT_EQ(calculate_vwap_no_vector(ob), 100.0f);
}

TEST(NoVector, two_levels_equal_quantity) {
    const order_book_t ob{.m_prices = {100.0f, 200.0f}, .m_quantities = {5.0f, 5.0f}};
    ASSERT_FLOAT_EQ(calculate_vwap_no_vector(ob), 150.0f);
}

TEST(NoVector, two_levels_unequal_quantity) {
    const order_book_t ob{.m_prices = {100.0f, 200.0f}, .m_quantities = {1.0f, 3.0f}};
    ASSERT_FLOAT_EQ(calculate_vwap_no_vector(ob), 175.0f);
}

TEST(NoVector, all_zero_quantities) {
    const order_book_t ob{.m_prices = {100.0f, 200.0f}, .m_quantities = {0.0f, 0.0f}};
    ASSERT_FLOAT_EQ(calculate_vwap_no_vector(ob), 0.0f);
}

TEST(NoVector, multiple_levels) {
    const order_book_t ob{
    ...
    ASSERT_FLOAT_EQ(calculate_vwap_no_vector(ob), expected);
}
