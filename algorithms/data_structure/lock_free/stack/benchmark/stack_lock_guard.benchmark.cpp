...
#include "stack_optimize_mutex.hpp"

#include <benchmark/benchmark.h>
#include <latch>

template<template<class> class TStack>
static void BM_ReadOnlyFromTheTop(benchmark::State& state)
{
    static TStack<int> stack;
    stack.push(1);
 ...
 */
BENCHMARK_TEMPLATE(BM_ReadOnlyFromTheTop, stack_lock_guard_t)   ->ThreadRange(1, 128);
BENCHMARK_TEMPLATE(BM_ReadOnlyFromTheTop, stack_optimize_mutex_t)->ThreadRange(1, 128);

BENCHMARK_TEMPLATE(BM_PushAndPop,      stack_lock_guard_t)      ->ThreadRange(1, 128);
BENCHMARK_TEMPLATE(BM_PushAndPop,      stack_optimize_mutex_t)  ->ThreadRange(1, 128);
